#!/usr/bin/env python3
"""
Multi-Pair CCI Strategy Backtest
...
def calculate_cci_signals(df, period=20, overbought=100, oversold=-100):
    """Calculate CCI signals"""
    try:
        # Calculate CCI
        cci_indicator = CCIIndicator(high=df['High'], low=df['Low'], close=df['Close'], window=period)
        df['cci'] = cci_indicator.cci()

        # Generate signals
        df['signal'] = 0

        # Oversold signal: CCI crosses above oversold level
        oversold_signal = (df['cci'] > oversold) & (df['cci'].shift(1) <= oversold)
        df.loc[oversold_signal, 'signal'] = 1

        # Overbought signal: CCI crosses below overbought level
        overbought_signal = (df['cci'] < overbought) & (df['cci'].shift(1) >= overbought)
        df.loc[overbought_signal, 'signal'] = -1

        return df

    except Exception as e:
        print(f"Error calculating CCI signals: {str(e)}")
        return df

def run_multi_pair_cci_backtest(pairs, timeframe='4h', period_days=720,
                               cci_period=20, overbought=100, oversold=-100,
                               risk_per_trade=0.015, stop_loss_pct=0.02, take_profit_pct=0.05):
    """Run multi-pair CCI backtest"""

    print("🎯 Multi-Pair CCI Strategy")
    print("=" * 80)
    print("🚀 Multi-Pair CCI Strategy")
    print(f"Pairs: {', '.join(pairs)}")
    print(f"Timeframe: {timeframe}, Period: {period_days} days")
    print("Target: 80+ trades with 67%+ win rate")
    print(f"CCI: Period={cci_period}, Overbought={overbought}, Oversold={oversold}")
    print("=" * 80)
    ...
    for pair in pairs:
        ...
        # Calculate CCI signals
        df = calculate_cci_signals(df, cci_period, overbought, oversold)
    ...
    with open('multi_pair_cci_results.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)

    print("\n💾 Results saved to multi_pair_cci_results.json")
...
if __name__ == "__main__":
    ...
    # Run the backtest
    results = run_multi_pair_cci_backtest(
        pairs=pairs,
        timeframe='4h',
        period_days=720,
        cci_period=20,
    ...
    print("\n✅ Multi-pair CCI backtest complete!")