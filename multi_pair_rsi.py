#!/usr/bin/env python3
"""
Multi-Pair RSI Mean Reversion Strategy
...
# RSI Parameters optimized for high win rate
RSI_PERIOD = 14
RSI_OVERBOUGHT = 70  # Conservative levels for quality
RSI_OVERSOLD = 30
...
def run_multi_pair_rsi_backtest():
    """Run RSI mean reversion backtest across multiple pairs"""
    all_trades = []
    ...
    print(f"🚀 Multi-Pair RSI Mean Reversion Strategy")
    ...
    for pair in PAIRS:
        ...
        for i in range(len(df)):
            ...
            buy_signal = rsi_val < RSI_OVERSOLD  # Buy when oversold
            sell_signal = rsi_val > RSI_OVERBOUGHT  # Sell when overbought
            ...
            if position:
                ...
                if position['type'] == 'LONG':
                    ...
                    if profit_pct <= -STOP_LOSS_PCT or profit_pct >= TAKE_PROFIT_PCT:
                        ...
                        balance += pnl
                        all_trades.append({
                            'id': pid, 'symbol': pair, 'strategy': 'Multi-Pair RSI',
    ...
    summary = {
        'strategy': 'Multi-Pair RSI Mean Reversion',
        'pairs': PAIRS,
        'timeframe': TIMEFRAME,
        'period_days': DAYS,
...
def main():
    print("🎯 Multi-Pair RSI Mean Reversion Strategy")
    print("="*80)

    # Run multi-pair RSI backtest
    trades, equity, summary = run_multi_pair_rsi_backtest()

    # Display results
    print(f"\n" + "="*80)
    print(f"📊 MULTI-PAIR RSI RESULTS")
    print(f"="*80)
    ...
    # Save results
    result_file = 'multi_pair_rsi_stoch_rsi.json'
    ...
    print(f"\n✅ Multi-pair RSI backtest complete!")
...
def plot_multi_pair_equity(equity, summary):
    ...
    fname = 'multi_pair_rsi_equity.png'
