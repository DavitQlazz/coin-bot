#!/usr/bin/env python3
"""
Multi-Pair Williams %R Strategy Backtest
...
def calculate_williams_r_signals(df, period=14, overbought=-20, oversold=-80):
    """Calculate Williams %R signals"""
    try:
        # Calculate Williams %R
        williams_r = WilliamsRIndicator(high=df['High'], low=df['Low'], close=df['Close'], lbp=period)
        df['williams_r'] = williams_r.williams_r()

        # Generate signals
        df['signal'] = 0

        # Oversold signal (buy): Williams %R crosses above oversold level
        oversold_signal = (df['williams_r'] > oversold) & (df['williams_r'].shift(1) <= oversold)
        df.loc[oversold_signal, 'signal'] = 1

        # Overbought signal (sell): Williams %R crosses below overbought level
        overbought_signal = (df['williams_r'] < overbought) & (df['williams_r'].shift(1) >= overbought)
        df.loc[overbought_signal, 'signal'] = -1

        return df

    except Exception as e:
        print(f"Error calculating Williams %R signals: {str(e)}")
        return df

def run_multi_pair_williams_r_backtest(pairs, timeframe='4h', period_days=720,
                                      williams_period=14, overbought=-20, oversold=-80,
                                      risk_per_trade=0.015, stop_loss_pct=0.02, take_profit_pct=0.05):
    """Run multi-pair Williams %R backtest"""

    print("🎯 Multi-Pair Williams %R Strategy")
    print("=" * 80)
    print("🚀 Multi-Pair Williams %R Strategy")
    print(f"Pairs: {', '.join(pairs)}")
    print(f"Timeframe: {timeframe}, Period: {period_days} days")
    print("Target: 80+ trades with 67%+ win rate")
    print(f"Williams %R: Period={williams_period}, Overbought={overbought}, Oversold={oversold}")
    print("=" * 80)
    ...
    for pair in pairs:
        ...
        # Calculate Williams %R signals
        df = calculate_williams_r_signals(df, williams_period, overbought, oversold)
    ...
    print("\n" + "=" * 80)
    print("📊 MULTI-PAIR WILLIAMS %R RESULTS")
    print("=" * 80)
    ...
    with open('multi_pair_williams_r_results.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)

    print("\n💾 Results saved to multi_pair_williams_r_results.json")
...
if __name__ == "__main__":
    ...
    # Run the backtest
    results = run_multi_pair_williams_r_backtest(
        pairs=pairs,
        timeframe='4h',
        period_days=720,
        williams_period=14,
    ...
    print("\n✅ Multi-pair Williams %R backtest complete!")