#!/usr/bin/env python3
"""
Hybrid Strategy: Stochastic RSI + ADX Trend Filter
Target: Higher quality trades by only trading in trending markets
"""
...
def run_hybrid_multi_pair_stoch_rsi_adx_backtest(pairs, timeframe='4h', period_days=720,
                                                rsi_period=14, stoch_period=14, k_period=3, d_period=3,
                                                adx_period=14, adx_threshold=20,
                                                overbought=80, oversold=20,
                                                risk_per_trade=0.015, stop_loss_pct=0.02, take_profit_pct=0.05):
    """Run hybrid multi-pair Stochastic RSI + ADX backtest"""

    print("🎯 Hybrid Strategy: Stochastic RSI + ADX Trend Filter")
    print("=" * 80)
    print("🚀 Hybrid Multi-Pair Stochastic RSI + ADX Strategy")
    print(f"Pairs: {', '.join(pairs)}")
    print(f"Timeframe: {timeframe}, Period: {period_days} days")
    print("Target: Higher quality trades by only trading in trending markets")
    ...
    print("\n" + "=" * 80)
    print("📊 HYBRID STOCH RSI + ADX RESULTS")
    print("=" * 80)
    ...
    # Save results
    results = {
        'strategy': 'Hybrid Stochastic RSI + ADX',
        'parameters': {
    ...
    with open('hybrid_stoch_rsi_adx_results.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)

    print("\n💾 Results saved to hybrid_stoch_rsi_adx_results.json")
...
if __name__ == "__main__":
    ...
    # Run the hybrid strategy optimized for maximum trades with quality filter
    results = run_hybrid_multi_pair_stoch_rsi_adx_backtest(
        pairs=pairs,
        timeframe='4h',     # Back to 4h for better data quality
        period_days=720,
        rsi_period=14,
        stoch_period=14,
        k_period=3,
        d_period=3,
        adx_period=14,
        adx_threshold=10,  # Very low threshold for maximum trades
        overbought=80,     # Standard levels for quality
        oversold=20,       # Standard levels for quality
        risk_per_trade=0.015,
        stop_loss_pct=0.02,
        take_profit_pct=0.05
    )

    print("\n✅ Hybrid Stochastic RSI + ADX backtest complete!")