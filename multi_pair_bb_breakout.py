#!/usr/bin/env python3
"""
Multi-Pair Bollinger Bands Breakout Strategy
...
def calculate_bollinger_breakout_signals(df, period=20, std=2.0):
    ...
    try:
        ...
        bullish_breakout = (df['Close'] > df['upper_band']) & (df['Close'].shift(1) <= df['upper_band'].shift(1))
        df.loc[bullish_breakout, 'signal'] = 1

        # Bearish breakout: Close breaks below lower band
        bearish_breakout = (df['Close'] < df['lower_band']) & (df['Close'].shift(1) >= df['lower_band'].shift(1))
        df.loc[bearish_breakout, 'signal'] = -1

        return df

    except Exception as e:
        print(f"Error calculating Bollinger Bands breakout signals: {str(e)}")
        return df

def run_multi_pair_bb_breakout_backtest(pairs, timeframe='4h', period_days=720,
                                       bb_period=20, bb_std=2.0,
                                       risk_per_trade=0.015, stop_loss_pct=0.02, take_profit_pct=0.05):
    """Run multi-pair Bollinger Bands breakout backtest"""

    print("🎯 Multi-Pair Bollinger Bands Breakout Strategy")
    print("=" * 80)
    print("🚀 Multi-Pair Bollinger Bands Breakout Strategy")
    ...
    for pair in pairs:
        ...
        # Calculate Bollinger Bands breakout signals
        df = calculate_bollinger_breakout_signals(df, bb_period, bb_std)
    ...
    print("\n" + "=" * 80)
    print("📊 MULTI-PAIR BOLLINGER BANDS BREAKOUT RESULTS")
    print("=" * 80)
    ...
    with open('multi_pair_bb_breakout_results.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)

    print("\n💾 Results saved to multi_pair_bb_breakout_results.json")
...
if __name__ == "__main__":
    ...
    results = run_multi_pair_bb_breakout_backtest(
        pairs=pairs,
        timeframe='4h',
        period_days=720,
        bb_period=20,
    ...
    print("\n✅ Multi-pair Bollinger Bands breakout backtest complete!")