#!/usr/bin/env python3
"""
Optimized Hybrid Strategy: Focus on Best Performing Pairs
Based on ADX + Stochastic RSI results, focusing on top performers
"""
...
def calculate_hybrid_stoch_rsi_adx_signals(df, rsi_period=14, stoch_period=14, k_period=3, d_period=3,
                                          adx_period=14, adx_threshold=10,
                                          overbought=80, oversold=20):
    """Calculate hybrid Stochastic RSI + ADX signals"""
    try:
        ...
        # Generate signals: Stochastic RSI signals only when ADX indicates trending market
        df['signal'] = 0

        # Long signal: ADX trending + Stochastic RSI oversold
        long_condition = ((df['adx'] > adx_threshold) &  # Trending market
                         (df['adx_pos'] > df['adx_neg']) &  # Uptrend
                         (df['stoch_rsi_k'] < oversold) & (df['stoch_rsi_d'] < oversold) &  # Oversold
                         (df['stoch_rsi_k'] > df['stoch_rsi_d']) &  # K crossing above D
                         (df['stoch_rsi_k'].shift(1) <= df['stoch_rsi_d'].shift(1))  # Crossover
                        )
        df.loc[long_condition, 'signal'] = 1

        # Short signal: ADX trending + Stochastic RSI overbought
        short_condition = ((df['adx'] > adx_threshold) &  # Trending market
                          (df['adx_neg'] > df['adx_pos']) &  # Downtrend
                          (df['stoch_rsi_k'] > overbought) & (df['stoch_rsi_d'] > overbought) &  # Overbought
...
def run_optimized_multi_pair_stoch_rsi_adx_backtest(pairs, timeframe='4h', period_days=720,
                                                   rsi_period=14, stoch_period=14, k_period=3, d_period=3,
                                                   adx_period=14, adx_threshold=10,
                                                   overbought=80, oversold=20,
                                                   risk_per_trade=0.015, stop_loss_pct=0.02, take_profit_pct=0.05):
    """Run optimized multi-pair Stochastic RSI + ADX backtest on best performing pairs"""

    print("🎯 Optimized Hybrid Strategy: Best Performing Pairs")
    print("=" * 80)
    print("🚀 Hybrid Stochastic RSI + ADX on Top Performing Pairs")
    print(f"Pairs: {', '.join(pairs)}")
    print(f"Timeframe: {timeframe}, Period: {period_days} days")
    print("Strategy: Focus on highest win rate pairs from previous testing")
    ...
    for pair in pairs:
        ...
        df = calculate_hybrid_stoch_rsi_adx_signals(df, rsi_period, stoch_period, k_period, d_period,
    ...
    print("\n" + "=" * 80)
    print("📊 OPTIMIZED BEST PAIRS RESULTS")
    print("=" * 80)
    ...
    if total_trades >= 15 and win_rate >= 50:
        print("🎉 EXCELLENT PERFORMANCE: 15+ trades with 50%+ win rate!")
    elif total_trades >= 10 and win_rate >= 55:
        print("🎯 STRONG PERFORMANCE: 10+ trades with 55%+ win rate!")
    elif win_rate >= 60:
        print(f"✅ High win rate ({win_rate:.1f}%) achieved!")
    else:
    ...
    # Save results
    results = {
        'strategy': 'Optimized Hybrid - Best Pairs',
        'parameters': {
    ...
    with open('optimized_best_pairs_results.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)

    print("\n💾 Results saved to optimized_best_pairs_results.json")
...
if __name__ == "__main__":
    ...
    # Run the optimized strategy on best performing pairs
    results = run_optimized_multi_pair_stoch_rsi_adx_backtest(
        pairs=best_pairs,
        timeframe='4h',
        period_days=720,
        rsi_period=14,
        ...
        d_period=3,
        adx_period=14,
        adx_threshold=10,  # Optimized threshold for trade frequency
        overbought=80,
        oversold=20,
    ...
    print("\n✅ Optimized best pairs strategy complete!")